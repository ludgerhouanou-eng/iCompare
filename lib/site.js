...
const amazonTag = process.env.AMAZON_TAG || TAG;
...
export function amazonLink({ asin = null, link = null } = {}) {
  ...
  if (link) {
    if (/[?&]tag=/.test(link)) return link;
    const sep = link.includes("?") ? "&" : "?";
    return `${link}${sep}tag=${encodeURIComponent(amazonTag)}`;
  }
  if (asin) {
    const query = new URLSearchParams({ tag: amazonTag });
    return `https://www.${amazonDomain}/dp/${asin}?${query.toString()}`;
  }
  return null;
}
